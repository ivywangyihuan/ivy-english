...
export function ExamVocabularyBridge(){
  ...
  function save(){const word=selection.trim();addVocabulary({word,module:"阅读",source:scope==="full"?"IELTS Reading · 完整 CBT":"IELTS Reading · 单 Passage",context:"机考阅读中划词收藏"});try{const key=`ivy-reading-v3-${scope}`;const current=JSON.parse(window.localStorage.getItem(key)??"{}") as {vocabulary?:string[]};const vocabulary=Array.from(new Set([...(current.vocabulary??[]),word]));window.localStorage.setItem(key,JSON.stringify({...current,vocabulary}))}catch{}setSelection("");window.getSelection()?.removeAllRanges()}
