  ...
  return (
    <div className="relative min-h-screen bg-background">
      ...
      <div className="fixed right-5 bottom-[calc(env(safe-area-inset-bottom)+5.5rem)] z-40 md:hidden"><div className="relative size-12">
        ...
        <Link to="/library" aria-label="打开资料库" onClick={() => setQuickMenuOpen(false)} className={cn("absolute right-[3.15rem] bottom-[3.35rem] flex size-11 touch-manipulation items-center justify-center rounded-full border border-border bg-paper text-foreground shadow-lift transition-all duration-200", quickMenuOpen ? "pointer-events-auto translate-x-0 translate-y-0 scale-100 opacity-100 delay-[35ms]" : "pointer-events-none translate-x-5 translate-y-5 scale-75 opacity-0")}><Library className="size-[18px]" strokeWidth={1.7} /></Link>
        <Link to="/settings" aria-label="打开设置" onClick={() => setQuickMenuOpen(false)} className={cn("absolute right-0 bottom-[4.75rem] flex size-11 touch-manipulation items-center justify-center rounded-full border border-border bg-paper text-foreground shadow-lift transition-all duration-200", quickMenuOpen ? "pointer-events-auto translate-x-0 translate-y-0 scale-100 opacity-100 delay-[70ms]" : "pointer-events-none translate-y-7 scale-75 opacity-0")}><Settings className="size-[18px]" strokeWidth={1.7} /></Link>
        <button type="button" aria-label={quickMenuOpen ? "关闭快捷菜单" : "打开快捷菜单"} aria-expanded={quickMenuOpen} onClick={() => setQuickMenuOpen((open) => !open)} className="absolute inset-0 flex size-12 touch-manipulation items-center justify-center rounded-full bg-primary text-primary-foreground shadow-lift transition-transform duration-200 active:scale-95">{quickMenuOpen ? <X className="size-5" strokeWidth={1.8} /> : <Ellipsis className="size-5" strokeWidth={2} />}</button>
      </div></div>
