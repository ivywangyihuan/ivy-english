...
const listeningModes=new Set(["video","dictation","shadowing","graded"]);
const readingModes=new Set(["article","academic","saved","material"]);
...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  const mode=typeof routeSearch.mode==="string"?routeSearch.mode:"";
  ...
  const workspaceSubject=pathname.startsWith("/speaking-workspace")?"speaking":listeningModes.has(mode)?"listening":readingModes.has(mode)?"reading":"writing";
  ...
  function mainActive(to: (typeof nav)[number]["to"]) { if (to === "/") return pathname === "/"; if (to === "/progress") return pathname === "/progress" || pathname === "/progress-detail"; return pathname === to || pathname.startsWith(`${to}/`); }
  function openCapture() { setQuickMenuOpen(false); setCaptureOpen(true); }
  if (isExamRoute) return <>{children}</>;
  if (isJournalImmersive) return <div className={`journal-workspace journal-canvas subject-${workspaceSubject} journal-subject-${workspaceSubject} min-h-screen`}>{children}</div>;
  ...
  return (
    <div className="journal-app journal-canvas relative min-h-screen bg-background">
      ...
      <nav className="fixed inset-x-0 bottom-0 z-30 border-t border-[color:var(--journal-line)] bg-[rgba(255,252,247,.96)] pb-[env(safe-area-inset-bottom)] backdrop-blur md:hidden"><div className="grid grid-cols-4">{nav.map((item) => {const active=mainActive(item.to);return <Link key={item.to} to={item.to} activeOptions={{ exact: item.to === "/" }} className={cn("flex touch-manipulation flex-col items-center gap-1 py-3 text-[11px] transition-colors duration-100",active?"text-[var(--journal-ink)]":"text-[#63709A]")}><item.icon className="size-5" strokeWidth={active?2:1.6}/>{item.mobileLabel}</Link>})}</div></nav>
      <div className="fixed right-5 bottom-[calc(env(safe-area-inset-bottom)+5.5rem)] z-40 md:hidden"><div className="relative size-12"><button type="button" aria-label="记录一次学习" onClick={openCapture} className={cn("absolute right-[4.35rem] bottom-0 flex size-11 touch-manipulation items-center justify-center border border-[var(--journal-line)] bg-[var(--journal-paper)] text-[var(--journal-ink)] shadow-[3px_3px_0_rgba(63,99,242,.10)] transition-all duration-200", quickMenuOpen ? "pointer-events-auto translate-x-0 translate-y-0 scale-100 opacity-100" : "pointer-events-none translate-x-8 scale-75 opacity-0")}><Plus className="size-[18px]" strokeWidth={1.8} /></button><Link to="/library" aria-label="打开资料库" onClick={() => setQuickMenuOpen(false)} className={cn("absolute right-[3.15rem] bottom-[3.35rem] flex size-11 touch-manipulation items-center justify-center border border-[var(--journal-line)] bg-[var(--journal-yellow)] text-[var(--journal-ink)] shadow-[3px_3px_0_rgba(255,93,177,.08)] transition-all duration-200", quickMenuOpen ? "pointer-events-auto translate-x-0 translate-y-0 scale-100 opacity-100 delay-[35ms]" : "pointer-events-none translate-x-5 translate-y-5 scale-75 opacity-0")}><Library className="size-[18px]" strokeWidth={1.7} /></Link><Link to="/settings" aria-label="打开设置" onClick={() => setQuickMenuOpen(false)} className={cn("absolute right-0 bottom-[4.75rem] flex size-11 touch-manipulation items-center justify-center border border-[var(--journal-line)] bg-[var(--journal-lavender)] text-[var(--journal-ink)] shadow-[3px_3px_0_rgba(63,99,242,.10)] transition-all duration-200", quickMenuOpen ? "pointer-events-auto translate-x-0 translate-y-0 scale-100 opacity-100 delay-[70ms]" : "pointer-events-none translate-y-7 scale-75 opacity-0")}><Settings className="size-[18px]" strokeWidth={1.7} /></Link><button type="button" aria-label={quickMenuOpen ? "关闭快捷菜单" : "打开快捷菜单"} aria-expanded={quickMenuOpen} onClick={() => setQuickMenuOpen((open) => !open)} className="absolute inset-0 flex size-12 touch-manipulation items-center justify-center rounded-full border border-[var(--journal-ink)] bg-[var(--journal-ink)] text-white shadow-[3px_3px_0_rgba(63,99,242,.20)] transition-transform duration-200 active:scale-95">{quickMenuOpen ? <X className="size-5" strokeWidth={1.8} /> : <Ellipsis className="size-5" strokeWidth={2} />}</button></div></div>
