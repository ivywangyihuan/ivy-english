...
import { GridPaper, JournalHeading, JournalPaper, MarkerNote, ReceiptCard, Stamp, TapeLabel } from "@/components/journal-ui";
...
const quickStart=[
  {title:"听力",hint:"真实英语 · 听写 · Shadowing",icon:Headphones,module:"listening" as const,tone:"aqua" as const,label:"LISTEN"},
  {title:"口语",hint:"自由表达 · IELTS Speaking",icon:Mic,module:"speaking" as const,tone:"mint" as const,label:"SPEAK"},
  {title:"阅读",hint:"文章阅读 · IELTS CBT",icon:BookOpenText,module:"reading" as const,tone:"yellow" as const,label:"READ"},
  {title:"写作",hint:"自由写作 · IELTS Writing",icon:PenLine,module:"writing" as const,tone:"lavender" as const,label:"WRITE"},
];
  ...
  return <div className="journal-canvas -mx-5 -mt-8 min-h-screen px-5 pb-8 pt-8 sm:-mx-8 sm:px-8 md:-mt-10 md:pt-10">
    <div className="mx-auto max-w-5xl space-y-9 sm:space-y-11">
      ...
      <div className="grid items-start gap-6 lg:grid-cols-[1.05fr_.95fr]">
        ...
        <JournalPaper tone="mint" tilt={1} className="p-5 sm:p-6">
      ...
      <section>
        ...
        <JournalPaper tone="paper" className="p-5 sm:p-7">
          ...
          <div className="mt-7 space-y-3">{weekBreakdown.map((item,index)=><div key={item.label} className="grid grid-cols-[42px_1fr_42px] items-center gap-3 text-xs"><span className="font-medium text-[var(--journal-ink)]">{item.label}</span><div className="h-2 overflow-hidden bg-[#EEF4FF]"><div className={index%2===0?"h-full bg-[var(--journal-blue)]":"h-full bg-[var(--journal-turquoise)]"} style={{width:`${Math.max(item.minutes?8:0,item.minutes/maxMinutes*100)}%`}}/></div><span className="text-right tabular-nums text-[#63709A]">{item.minutes}m</span></div>)}</div>
        </JournalPaper>
      </section>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <section className="hidden sm:block">
          ...
          <GridPaper className="border border-[color:var(--journal-line)] p-4"><div className="grid grid-cols-2 gap-3">{recentQuestions.map((question,index)=><JournalPaper key={question.id} tone={index%2===0?"paper":"peach"} className="p-4 shadow-none"><Link to="/question" search={{id:question.id}} className="relative z-10 block"><div className="flex items-center justify-between gap-2"><span className="text-[9px] font-semibold tracking-[0.14em] text-[var(--journal-turquoise)]">{question.module}{question.part?` · ${question.part}`:""}</span><span className="text-[9px] tabular-nums text-[#63709A]">{question.recentDate??"近期"}</span></div><p className="mt-3 line-clamp-3 text-xs leading-5 text-[var(--journal-ink)]">{question.title}</p></Link></JournalPaper>)}</div></GridPaper>
        </section>
      </div>
    </div>
  </div>;
