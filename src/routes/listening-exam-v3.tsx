...
export const Route = createFileRoute("/listening-exam-v3")({
...
function ListeningExamPage() {
  ...
  const storageKey = `ivy-listening-v3-${scope}`;

  const timing = useMemo(() => {
    const rawDurations = activeSection.script.map((line) => Math.max(3, line.split(/\s+/).length / 2.1 + 0.7));
    const rawTotal = rawDurations.reduce((sum, value) => sum + value, 0);
    const scale = activeSection.estimatedSeconds / rawTotal;
  ...
  useEffect(() => { window.localStorage.setItem(storageKey, JSON.stringify({ answers, volume })); }, [answers, storageKey, volume]);
  ...
  useEffect(() => {
    ...
    audioClockRef.current = window.setInterval(() => setAudioElapsed((value) => Math.min(activeSection.estimatedSeconds, value + 1)), 1000);
    return () => { if (audioClockRef.current) window.clearInterval(audioClockRef.current); audioClockRef.current = null; };
  }, [activeSection.estimatedSeconds, audioState]);
  ...
  const score = useMemo(() => allQuestions.reduce((total, question) => {
  ...
  }, 0), [allQuestions, answers]);

  function speakSegment(index: number, generation: number) {
    if (!("speechSynthesis" in window) || index >= activeSection.script.length) {
      setAudioState("finished");
      setAudioElapsed(activeSection.estimatedSeconds);
      return;
    }
    setCurrentSegment(index);
    const utterance = new SpeechSynthesisUtterance(activeSection.script[index]);
    ...
    utterance.onend = () => {
      ...
      const next = index + 1;
      if (next >= activeSection.script.length) {
        setAudioState("finished");
        setAudioElapsed(activeSection.estimatedSeconds);
      } else {
        setAudioElapsed(Math.round(timing.starts[next] ?? 0));
        speakSegment(next, generation);
      }
    };
    utterance.onerror = () => { if (generation === generationRef.current) setAudioState("paused"); };
    window.speechSynthesis.speak(utterance);
  }

  function playFrom(index: number, displaySeconds?: number) {
    if (!("speechSynthesis" in window)) return;
    generationRef.current += 1;
    const generation = generationRef.current;
    window.speechSynthesis.cancel();
    const safeIndex = Math.min(activeSection.script.length - 1, Math.max(0, index));
    setAudioElapsed(Math.max(0, Math.min(activeSection.estimatedSeconds, displaySeconds ?? Math.round(timing.starts[safeIndex] ?? 0))));
  ...
  function startAudio() {
    ...
    playFrom(audioState === "finished" ? 0 : currentSegment, audioState === "finished" ? 0 : audioElapsed);
  }
  ...
  function indexForSecond(target: number) {
    let index = 0;
    for (let i = 0; i < timing.starts.length; i += 1) if (timing.starts[i] <= target) index = i;
    return index;
  }

  function seekTo(target: number) {
    if (mode !== "familiarisation") return;
    const safe = Math.min(activeSection.estimatedSeconds - 1, Math.max(0, target));
    playFrom(indexForSecond(safe), safe);
  }

  function seekBy(delta: number) {
    seekTo(audioElapsed + delta);
  }
  ...
  function jumpToQuestion(number: number) {
    const sectionIndex = sections.findIndex((section) => section.questions.some((question) => question.number === number));
    if (sectionIndex >= 0) changeSection(sectionIndex);
    window.setTimeout(() => document.getElementById(`listening-q-${number}`)?.scrollIntoView({ behavior: "smooth", block: "center" }), 80);
  }
  ...
  return <div className="min-h-screen bg-[#f2f2f2] text-[#222]">
    ...
    <div className="hidden h-screen flex-col overflow-hidden bg-white lg:flex">
      ...
      {scope === "full" ? <div className="flex h-11 shrink-0 border-b border-[#ccc] bg-white px-4">{sections.map((section, index) => <button key={section.id} type="button" onClick={() => changeSection(index)} className={cn("border-b-2 px-5 text-xs font-semibold", activeSectionIndex === index ? "border-[#2b5874] bg-[#f4f8fa] text-[#21465d]" : "border-transparent text-[#666] hover:bg-[#f5f5f5]")}>Section {index + 1}<span className="ml-2 font-normal text-[#888]">{section.questions[0]?.number}–{section.questions.at(-1)?.number}</span></button>)}</div> : null}
      <div className="shrink-0 border-b border-[#ccc] bg-white px-5 py-3">
        <div className="flex items-center gap-3">
          <div className="flex size-9 shrink-0 items-center justify-center rounded-full bg-[#edf3f6] text-[#355f7a]"><Volume2 className="size-4" /></div>
          <div className="min-w-0 flex-1">
            <div className="mb-1 flex items-center justify-between text-[10px] text-[#666]"><span>{audioState === "idle" ? "Audio not started" : audioState === "playing" ? "Playing" : audioState === "paused" ? "Paused" : "Finished"}</span><span>{formatTime(sliderValue)} / ~{formatTime(activeSection.estimatedSeconds)}</span></div>
            {mode === "familiarisation" ? <input type="range" min={0} max={Math.max(1, activeSection.estimatedSeconds)} step={1} value={sliderValue} onPointerDown={() => { setScrubbing(true); setScrubSeconds(audioElapsed); }} onChange={(event) => setScrubSeconds(Number(event.target.value))} onPointerUp={(event) => { const value = Number(event.currentTarget.value); setScrubbing(false); seekTo(value); }} onKeyUp={(event) => seekTo(Number((event.currentTarget as HTMLInputElement).value))} className="block h-4 w-full cursor-pointer accent-[#557b92]" aria-label="拖动听力播放进度" /> : <div className="h-1.5 overflow-hidden rounded-full bg-[#ddd]"><div className="h-full bg-[#557b92]" style={{ width: `${Math.min(100, (audioElapsed / activeSection.estimatedSeconds) * 100)}%` }} /></div>}
          </div>
